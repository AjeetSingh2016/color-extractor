  ...
  return (
    <div className={`min-h-screen ${currentTheme.background} transition-colors duration-500 font-sans`}>
      <Head>
        ...
        <meta name="description" content="The ultimate color toolkit for designers and developers" />
      </Head>
      ...
      {isMenuOpen && (
        ...
        >
          <nav className="flex flex-col space-y-4">
            ...
            <button className={`rounded-full px-5 py-3 ${currentTheme.button} text-center mt-4`}>
              Sign Up
            </button>
          </nav>
        </motion.div>
      ...
      <div className="relative pt-20">
        ...
        <section className="py-20 px-4" id="tools">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              {[
                { icon: <Palette className="h-8 w-8" />, title: 'All-in-One Toolkit', desc: 'Everything you need for color in one place, no more switching between tools' },
                { icon: <Accessibility className="h-8 w-8" />, title: 'Accessibility-Focused', desc: 'Built with WCAG standards to ensure your designs are accessible to everyone' },
                { icon: <Rocket className="h-8 w-8" />, title: 'Instant & Free', desc: 'No sign-up required, just start using our powerful tools immediately' },
                { icon: <Code className="h-8 w-8" />, title: 'Developer-Friendly', desc: 'Full API access and code snippets for easy integration into your workflow' }
              ].map((benefit, index) => (
                <motion.div
                  key={index}
                ...
                >
                  <div className={`mb-5 ${currentTheme.primary} bg-opacity-10 rounded-xl p-3 inline-block`}>{benefit.icon}</div>
                  <h3 className={`text-xl font-semibold mb-3 ${currentTheme.headline}`}>{benefit.title}</h3>
        ...
        <section className={`py-24 px-4 ${isDarkMode ? 'bg-slate-800/50' : 'bg-indigo-50/50'}`} id="resources">
          <div className="max-w-6xl mx-auto">
            <motion.div
              initial="hidden"
              whileInView="show"
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                { 
                  icon: <div className="bg-gradient-to-br from-indigo-500 to-blue-500 p-3 rounded-xl"><Palette className="h-8 w-8 text-white" /></div>,
                  name: 'Gradient Generator',
                  desc: 'Create beautiful gradients with our intuitive tool. Perfect for backgrounds, buttons, and UI elements.',
                  color: 'from-indigo-500 to-blue-500'
                },
                { 
                  icon: <div className="bg-gradient-to-br from-emerald-500 to-teal-500 p-3 rounded-xl"><Accessibility className="h-8 w-8 text-white" /></div>,
                  name: 'Contrast Checker', 
                  desc: 'Ensure your color combinations meet WCAG accessibility standards for better usability.',
                  color: 'from-emerald-500 to-teal-500'
                },
                { 
                  icon: <div className="bg-gradient-to-br from-fuchsia-500 to-purple-500 p-3 rounded-xl"><Rocket className="h-8 w-8 text-white" /></div>,
                ...
                >
                  ...
                  <div className="relative z-10">
                    ...
                    <h3 className={`text-2xl font-semibold mb-3 ${currentTheme.headline}`}>{tool.name}</h3>
                    <p className={`${currentTheme.paragraph} mb-6`}>{tool.desc}</p>
                    <div className="flex space-x-4 items-center">
                      ...
                      <a href="#" className={`text-sm ${currentTheme.primary} font-medium hover:underline`}>Learn more →</a>
                    </div>
                  </div>
                  <div className={`absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r ${tool.color}`}></div>
                </motion.div>
              ))}
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {[
                ...
                >
                  ...
                  <p className={`text-lg mb-6 ${currentTheme.paragraph}`}>{testimonial.quote}</p>
                  <div className="flex items-center">
                    <div className="w-12 h-12 rounded-full bg-gradient-to-br from-indigo-500 to-purple-500"></div>
                    <div className="ml-4">
                      <h4 className={`font-semibold ${currentTheme.headline}`}>{testimonial.author}</h4>
                      <p className={`text-sm ${currentTheme.paragraph}`}>{testimonial.role}</p>
                    </div>
                  </div>
                </motion.div>
              ))}
        ...
        {/* CTA Section */}
        <section className={`py-24 px-4 ${isDarkMode ? 'bg-indigo-900/20' : 'bg-indigo-50'}`}>
          <div className="max-w-4xl mx-auto text-center">
            <motion.div
              ...
              transition={{ duration: 0.8 }}
            >
              <h2 className={`text-4xl md:text-5xl font-bold mb-6 ${currentTheme.headline}`}>
